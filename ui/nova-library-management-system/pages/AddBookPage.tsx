...
const AddBookPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || 'Error creating book');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-10 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-slate-900">Add New <span className="text-gradient">Book</span></h1>
          <p className="text-slate-500 font-medium">Add a fresh title to your collection.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            {loading ? 'Adding Book...' : 'Save Book to Catalog'}
          </button>
        </form>
      </div>
    </div>
